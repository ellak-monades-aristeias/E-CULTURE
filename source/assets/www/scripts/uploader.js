...
function doFileUpload(){
    ...
    if (filecount > 0)
    {
        ...
        for (i = 1; i <=  filecount; i++)
        {
            ...
            previewblock += "<td align='center' width='20%' ><i class='icon-trash icon-2x' style='cursor:pointer' onclick='deletefile(\""+fieldname+"\", "+i+")'> </i></td></tr></table>"+
    ...
    new AjaxUpload(button, {
        ...
        onSubmit : function(file, ext){
           ...
           if (filecount >= maxfiles)
            {
                $('#notice').html('<div class="alert in fade alert-error">'+translt.errorNoMoreFiles+'</div>');
                return false;
            }
           ...
           var allowSubmit = false;
            ...
            {
                $('#notice').html('<div class="alert in fade alert-error">'+translt.errorOnlyAllowed.replace('%s',$('#'+fieldname+'_allowed_filetypes').val())+'</div>');
                return false;
            }
        ...
        onComplete: function(file, response){
            ...
            if (metadata.success)
            {
                $('#notice').html('<div class="alert in alert-block fade alert-success">'+metadata.msg+'</div>');
                ...
                previewblock += "<td  align='center' width='20%'><i class='icon-trash icon-2x' style='cursor:pointer' onclick='deletefile(\""+fieldname+"\", "+count+")'> </i></td>"+
                                        "</tr></table>"+
                ...
                if (filecount >= maxfiles)
                    $('#notice').html('<div class="alert in alert-block fade alert-success">'+translt.errorTooMuch+'</div>');
            }
            else
            {
...
function deletefile(fieldname, count) {
    ...
    xmlhttp.onreadystatechange=function()
    {
        ...
        {
            $('#notice').html('<div class="alert in alert-block fade alert-success">'+xmlhttp.responseText+'</div>');
            setTimeout(function() {
